...
class Camera {
  ...
  pan(dx, dy) {
    ...
    const dyRange = [-panBound - (m[7] + 1) / m[4], panBound - (m[7] - 1) / m[4]];
    const dxRange = [-panBound - (m[6] + 1) / m[0], panBound - (m[6] - 1) / m[0]];

    const dxClamped = clamp(dx, dxRange);
    const dyClamped = clamp(dy, dyRange);
    if (Math.abs(dxClamped) <= EPSILON && Math.abs(dyClamped) <= EPSILON) return;
  ...
  */
  ...
  localPosition(target, canvasX, canvasY, projectionInvTF) {
    ...
    */
    ...
    const pos = vec2.fromValues(2 * (canvasX / width) - 1, -2 * (canvasY / height) + 1);
    if (projectionInvTF) {
  ...
  mousePan(e, projectionTF) {
    ...
    const pos = this.localPosition(this.canvas, e.clientX, e.clientY, projectionInvTF);
    const prev = this.localPosition(this.canvas, this.prevEvent.clientX, this.prevEvent.clientY, projectionInvTF);
  ...
  wheelZoom(e, projectionTF) {
    ...
    const projectionInvTF = mat3.invert(scratch0, projectionTF);
    const pos = this.localPosition(this.canvas, clientX, clientY, projectionInvTF);
