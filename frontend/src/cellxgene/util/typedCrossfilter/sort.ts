...
export function sortIndex(index, source) {
  if (isFpTypedArray(source)) return quicksortFloatsIndirect(index, source, 0, index.length - 1);
...
//
function lowerBoundNonFloatIndirect(valueArray, indexArray, value, first, last) {
  let lfirst = first;
  let llast = last;
  // this is just a binary search
  while (lfirst < llast) {
...
//
function upperBoundNonFloatIndirect(valueArray, indexArray, value, first, last) {
  let lfirst = first;
  let llast = last;
  // this is just a binary search
  while (lfirst < llast) {
