...
import { isTypedArray, isArrayOrTypedArray, callOnceLazy, memoize } from "./util";
import { summarizeContinuous, summarizeCategorical as _summarizeCategorical } from "./summarize";
import {
...
class Dataframe {
  ...
  **/
  ...
  constructor(
    ...
    rowIndex: any = null,
    colIndex: any = null,
    __columnsAccessor: any[] = [] // private interface
  ) {
    /*
  ...
  static __errorChecks(dims, columnarData, rowIndex, colIndex) {
    ...
    if (nCols !== columnarData.length || !columnarData.every((c) => c.length === nRows)) {
      throw new RangeError("Dataframe dimension does not match provided data shape");
    }
    if (nRows !== rowIndex.size()) {
      throw new RangeError("Dataframe rowIndex must have same size as underlying data");
    }
    if (nCols !== colIndex.size()) {
      throw new RangeError("Dataframe colIndex must have same size as underlying data");
    }
  }
  ...
  static __compileColumn(column, getRowByOffset, getRowByLabel) {
    ...
    */
    const summarizeCategorical = callOnceLazy(() => _summarizeCategorical(column));
    const summarize = callOnceLazy(() =>
      isTypedArray(column) ? summarizeContinuous(column) : summarizeCategorical(column)
    );
  ...
  __compile(accessors) {
    ...
    */
    const { getOffset: getRowByOffset, getLabel: getRowByLabel } = this.rowIndex;
  ...
  clone() {
    ...
    */
    return new Dataframe(this.dims, [...this.__columns], this.rowIndex, this.colIndex, [...this.__columnsAccessor]);
  }
  ...
  withColsFrom(dataframe, labels?) {
    ...
    * labels: columns to pull from `dataframe` and combine with `this`.  If falsey,
    ...
    const { rowIndex } = this;
    const columns = [...this.__columns, ...srcOffsets.map((i) => dataframe.__columns[i])];
    const colIndex = this.colIndex.withLabels(dstLabels);
    const columnsAccessor = [...this.__columnsAccessor, ...srcOffsets.map((i) => dataframe.__columnsAccessor[i])];
  ...
  dropCol(label) {
    ...
    */
    ...
    return new Dataframe(dims, columns, this.rowIndex, colIndex, columnsAccessor);
  }
  ...
  renameCol(oldLabel, newLabel) {
    ...
    */
    ...
    return new Dataframe(this.dims, columns, this.rowIndex, colIndex, columnsAccessor);
  }
  ...
  replaceColData(label, newColData) {
    ...
    */
    ...
    return new Dataframe(this.dims, columns, this.rowIndex, this.colIndex, columnsAccessor);
  }

  static empty(rowIndex = null, colIndex: any = null) {
  ...
  __subset(rowOffsets, colOffsets, withRowIndex) {
    ...
    if (colOffsets) {
      let colLabels;
      [colLabels, colOffsets] = getSortedLabelAndOffsets(colOffsets, this.colIndex);
    ...
    if (rowOffsets) {
      let rowLabels;
      [rowLabels, rowOffsets] = getSortedLabelAndOffsets(rowOffsets, this.rowIndex);
  ...
  isubsetMask(rowMask, colMask: any = null, withRowIndex = null) {
    /*
    ...
    */
    ...
    if ((rowMask && rowMask.length !== nRows) || (colMask && colMask.length !== nCols)) {
