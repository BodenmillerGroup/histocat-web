...
export function histogramCategorical(column, by) {
  ...
  return by ? _histogramCategoricalBy(column, by) : _histogramCategorical(column);
}
...
export function histogramContinuous(column, bins = 40, domain = [0, 1], by) {
  ...
  return by ? _histogramContinuousBy(column, bins, min, max, by) : _histogramContinuous(column, bins, min, max);
}
