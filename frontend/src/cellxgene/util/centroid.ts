...
const getCoordinatesByLabel = (
  ...
  const { categoryValueIndices, categoryValueCounts } = categoricalSelection?.[categoryName] || {};
  ...
  for (let i = 0, len = categoryArray.length; i < len; i += 1) {
    ...
    if (labelIndex !== undefined && !(isUserAnno && label === unassignedCategoryLabel)) {
      ...
      coords.hasFinite = coords.hasFinite || (Number.isFinite(layoutXArray[i]) && Number.isFinite(layoutYArray[i]));
...
const calcMedianCentroid = (
  ...
  dataMap.forEach((coords, label) => {
    ...
    if (coords.length > 0 && coords.hasFinite) {
      const calculatedCoordinates: any[] = [];
...
const hashMedianCentroid = (
  ...
  return `${obsAnnotations.__id}+${obsLayout.__id}:${categoryName}:${layoutDimNames}:${Object.keys(
    categorySelection
