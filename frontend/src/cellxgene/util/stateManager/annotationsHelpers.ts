...
export function removeObsAnnoSchema(schema, name) {
  ...
  if (!_isUserAnnotation(schema, name)) throw new Error("removing non-user-defined schema");
...
export function removeObsAnnoCategory(schema, name, category) {
  ...
  if (!_isUserAnnotation(schema, name)) throw new Error("unable to modify read-only schema");
  if (category === unassignedCategoryLabel) throw new Error("may not remove unassigned category label");
...
export function allHaveLabelByMask(df, colName, label, mask) {
  ...
  if (!col) return false;
  if (df.length !== mask.length) throw new RangeError("mismatch on mask length");
...
export function createWritableAnnotationDimensions(world, crossfilter) {
  ...
  const writableAnnotations = schema.annotations.obs.columns.filter((s) => s.writable).map((s) => s.name);
  ...
  crossfilter = writableAnnotations.reduce((xflt, anno) => {
    ...
    return xflt.addDimension(dimName, "enum", obsAnnotations.col(anno).asArray());
  }, crossfilter);
  return crossfilter;
}
