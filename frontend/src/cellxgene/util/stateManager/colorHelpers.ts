...
create new colors state object.   Paramters:
  ...
  -
*/
export function createColors(world, colorMode: string | null = null, colorAccessor = null) {
  switch (colorMode) {
...
function createColorsByCategoricalMetadata(world, accessor) {
  ...
  const scale = d3.scaleSequential(interpolateRainbow).domain([0, categories.length]);
...
export const checkIfColorByDiffexpAndResetColors = (prevControls, state, prevWorld) => {
