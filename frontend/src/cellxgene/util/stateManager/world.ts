...
function templateWorld() {
  const obsAnnotations = Dataframe.empty();
  const varAnnotations = Dataframe.empty();
  const obsLayout = Dataframe.empty();
  const varData = Dataframe.empty(null, new KeyIndex());
  return {
...
function clipDataframe(
  ...
  upperQuantile,
  quantileF,
  clipPredicate = (df, colIdx, colLabel) => true,
  value = Number.NaN
) {
  /*
...
function setClippedDataframes(world) {
  ...
  const varDataQuantile = (label, q) => world.unclipped.varData.col(label).summarize().percentiles[100 * q];
...
export function createWorldBySelection(universe, world, crossfilter) {
  ...
  newWorld.unclipped.obsAnnotations = world.unclipped.obsAnnotations.isubsetMask(mask);
...
function deduceDimensionType(attributes, fieldName) {
  ...
  if (type === "string" || type === "categorical" || type === "boolean") {
    ...
    */
    console.error(`Warning - REST API returned unknown metadata schema (${type}) for field ${fieldName}.`);
...
export function createObsDimensions(crossfilter, world, XYdimNames) {
  ...
  */
  ...
  const annoList = schema.annotations.obs.columns.filter((anno) => anno.name !== indexName);
