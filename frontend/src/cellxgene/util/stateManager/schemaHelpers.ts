...
export function indexEntireSchema(schema) {
  ...
  schema.annotations.obsByName = fromEntries(schema.annotations.obs.columns.map((v) => [v.name, v]));
  schema.annotations.varByName = fromEntries(schema.annotations.var.columns.map((v) => [v.name, v]));
  schema.layout.obsByName = fromEntries(schema.layout.obs.map((v) => [v.name, v]));
  schema.layout.varByName = fromEntries(schema.layout.var.map((v) => [v.name, v]));
...
export function removeObsAnnoColumn(schema, name) {
  ...
  newSchema.annotations.obs.columns = schema.annotations.obs.columns.filter((v) => v.name !== name);
...
export function removeObsAnnoCategory(schema, name, category) {
  ...
  if (!categories) throw new Error("column does not exist or is not categorical");
...
export function addObsAnnoCategory(schema, name, category) {
  ...
  catAnno.categories = catLabelSort(catAnno.writable, [...catAnno.categories, category]);
  return newSchema;
}
