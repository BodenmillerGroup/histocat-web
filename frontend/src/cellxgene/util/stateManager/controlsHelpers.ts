...
import { userDefinedDimensionName, diffexpDimensionName } from "../nameCreators";

export function maxCategoryItems(config) {
  return config.parameters?.["max-category-items"] ?? globals.configDefaults.parameters["max-category-items"];
}
...
function topNCategories(colSchema, summary, N) {
  ...
  const sortIndex = fillRange(new Array(categories.length)).sort((a, b) => counts[b] - counts[a]);
  const topNindices = new Set(sortIndex.slice(0, N));

  const _topNCategories: any[] = [];
  const topNCounts: any[] = [];
  ...
  */
  ...
  return columns
    ...
    .filter((colSchema) => {
      ...
      const isSelectableType = type === "string" || type === "boolean" || type === "categorical";
...
export function createCategoricalSelection(world, names) {
  ...
  const res = names.reduce((acc, name) => {
    ...
    */
    ...
    const [categoryValues, categoryValueCounts] = topNCategories(colSchema, summary, N);
...
*/
export function createGeneDimensions(userDefinedGenes, diffexpGenes, world, crossfilter) {
  crossfilter = userDefinedGenes.reduce(
    (xflt, gene) =>
      xflt.addDimension(userDefinedDimensionName(gene), "scalar", world.varData.col(gene).asArray(), Float32Array),
    crossfilter
  );
  crossfilter = diffexpGenes.reduce(
    (xflt, gene) =>
      xflt.addDimension(diffexpDimensionName(gene), "scalar", world.varData.col(gene).asArray(), Float32Array),
    crossfilter
  );
  return crossfilter;
}
...
export function pruneVarDataCache(varData, needed) {
  ...
  */
  ...
  if (unused.length > 0) {
    ...
    const numToDrop = unused.length < numOverWatermark ? unused.length : numOverWatermark;
