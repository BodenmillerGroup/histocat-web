<template>
  <v-card tile class="ma-6 pa-1">
    ...
    <v-card-actions>
      <v-btn color="primary" :to="{ name: 'main-group-experiment', params: { experimentId: experiment.id } }">
        Open
      </v-btn>
      <v-spacer></v-spacer>
      <v-tooltip bottom v-if="isOwner || hasAdminAccess">
        <template v-slot:activator="{ on }">
          <v-btn icon v-on="on" :to="{ name: 'main-group-experiments-edit', params: { experimentId: experiment.id } }">
