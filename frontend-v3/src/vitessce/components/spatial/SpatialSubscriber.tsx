...
type SpatialSubscriberProps = {
  ...
  subobservationsPluralLabelOverride?: string;
  theme: string;
 ...
 */
export default function SpatialSubscriber(props: SpatialSubscriberProps) {
  const {
    uuid,
    coordinationScopes,
    removeGridComponent,
  ...
  const [isReady, setItemIsReady, resetReadyItems] = useReady(SPATIAL_DATA_TYPES);
  ...
  const layers = useMemo(
    () => [
      ...
      ...(rasterLayers ? rasterLayers.map((l: any) => ({ ...l, type: "raster" })) : []),
    ],
  ...
  const cellColors = useMemo(
    ...
    [cellColorEncoding, geneSelection, mergedCellSets, cellSetColor, cellSetSelection, expressionData, attrs]
  );

  const cellSelection = useMemo(() => Array.from(cellColors.keys()), [cellColors]);

  const getCellInfo = (cellId: string) => {
