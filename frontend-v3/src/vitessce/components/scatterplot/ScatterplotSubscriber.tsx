 ...
 */
export default function ScatterplotSubscriber(props: ScatterplotSubscriberProps) {
  const {
    uuid,
    coordinationScopes,
    removeGridComponent,
  ...
  const [isReady, setItemIsReady, resetReadyItems] = useReady(SCATTERPLOT_DATA_TYPES);
  ...
  const [expressionData] = useGeneSelection(loaders, dataset, setItemIsReady, false, geneSelection);
  const [attrs] = useExpressionAttrs(loaders, dataset, setItemIsReady, addUrl, false);
  const [cellRadiusScale, setCellRadiusScale] = useState<number>(0.2);
  ...
  const cellColors = useMemo(
    ...
    [cellColorEncoding, geneSelection, mergedCellSets, cellSetSelection, cellSetColor, expressionData, attrs]
  );
  ...
  useEffect(() => {
    ...
    if (cellValues?.length) {
      const cellCoordinates = Object.values(cells).map((c: any) => c.mappings[mapping]);
  ...
  return (
    ...
    >
      <Scatterplot
        ...
        setViewState={({ zoom: newZoom, target }: { zoom: number; target: any }) => {
          setZoom(newZoom);
          setTargetX(target[0]);
          setTargetY(target[1]);
          setTargetZ(target[2]);
