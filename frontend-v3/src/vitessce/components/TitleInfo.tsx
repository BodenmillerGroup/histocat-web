...
const useStyles = makeStyles((theme) => ({
  iconButton: {
    ...
    borderRadius: "2px",
    "&:hover": {
      backgroundColor: (theme.palette as any).primaryBackgroundLight,
    },
    "&:first-child": {
      marginLeft: "0.75em",
    },
...
function SettingsIconWithArrow({ open }: { open: boolean }) {
  return (
    <>
      <SettingsIcon />
...
type PlotOptionsProps = {
  options: any;
};

function PlotOptions(props: PlotOptionsProps) {
  const { options } = props;
...
function CloudDownloadIconWithArrow({ open }: { open: boolean }) {
  return (
    <>
      <CloudDownloadIcon />
      {open ? <ArrowDropUpIcon /> : <ArrowDropDownIcon />}
...
function DownloadOptions(props: DownloadOptionsProps) {
  const { urls } = props;
  const [open, toggle] = useReducer((v) => !v, false);
  const classes = useStyles();
  return (
...
function ClosePaneButton(props: ClosePaneButtonProps) {
  const { removeGridComponent } = props;
  const classes = useStyles();
  return (
    <IconButton onClick={removeGridComponent} size="small" className={classes.iconButton} title="close">
      <CloseIcon />
    </IconButton>
  );
}
      ...
      <div className="title d-flex justify-content-between align-items-baseline">
        ...
        <span className="details pl-2 align-items-end">
          <span className="d-flex justify-content-between">
            {info}
            {options && <PlotOptions options={options} />}
            {urls && urls.length > 0 && <DownloadOptions urls={urls} />}
