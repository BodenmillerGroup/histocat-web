 ...
 */
function getBaseLayerId(layerId: string) {
  return `base-${layerId}`;
}
 ...
 * @returns {string} The base layer ID.
 */
function getSelectedLayerId(layerId: string) {
...
export function getSelectionLayers(
  ...
  return [new SelectionLayer({
    ...
    onSelect: ({ pickingInfos }: any) => {
      const cellIds = pickingInfos.map((cellObj: any[]) => cellObj[0]);
 ...
 */
export function overlayBaseProps(props: any) {
  const {
