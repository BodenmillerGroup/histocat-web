...
export default class SelectionLayer extends CompositeLayer<any, any> {
  _selectPolygonObjects(coordinates: any[]) {
    ...
    const flippedCoordinates = (flipY
      ? coordinates.map(poly => poly.map((p: number[]) => ([p[0], -p[1]])))
      : coordinates);
    ...
    cellsQuadTree.visit((node: any, x0: number, y0: number, x1: number, y1: number) => {
