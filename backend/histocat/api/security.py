...
from histocat.modules.user import service as user_service
...
def get_admin(user: UserModel = Security(get_user)):
    if not user.is_active or not user.is_admin:
        raise HTTPException(status_code=HTTP_401_UNAUTHORIZED, detail="User doesn't have enough privileges")
    return user
...
def get_active_member(member: MemberModel = Security(get_group_member)):
    if not member.is_active:
        raise HTTPException(status_code=HTTP_401_UNAUTHORIZED, detail="Group member account is deactivated")
    return member
