...
def get_by_experiment_uid(session: Session, *, experiment_id: int, uid: str) -> Optional[Slide]:
    return session.query(Slide).filter(Slide.uid == uid, Slide.experiment_id == experiment_id).first()
